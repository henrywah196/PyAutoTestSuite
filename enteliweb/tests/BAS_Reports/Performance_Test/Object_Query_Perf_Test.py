...
# Global Settings
JSON_FILE_LOCATION = os.path.abspath(os.path.join(os.path.dirname(__file__), "Object_Query_Perf_Test.json"))
...
class TestCase(TestCaseTemplate):
    ...
    def testMain(self, testData):
        ...
        finishTime = currentTime + datetime.timedelta(seconds=timeOut)
        result = self.testingReport.generatingReport(timeout=600)
        if not result:
            if self.testingReport.isReportUITimeOut():
                Macros.SelectReportInstance("Building Automation\\Object Query\\%s"%reportName)
            while currentTime <= finishTime:
                result = self.testingReport.generatingReport(timeout=600, clickRun=False)
                if result:
                    break
                else:
                    currentTime = datetime.datetime.now()
        ...
        '''
        Macros.GenerateReport(self.testingReport, "Building Automation\\Object Query\\%s"%reportName, timeOut=600)
