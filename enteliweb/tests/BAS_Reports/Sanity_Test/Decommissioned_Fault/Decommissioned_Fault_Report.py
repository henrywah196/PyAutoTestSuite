...
# Global Settings
JSON_FILE_LOCATION = os.path.abspath(os.path.join(os.path.dirname(__file__), "Decommissioned_Fault_Report.json"))
...
class TestCase(TestCaseTemplate):
    ...
    def testMain(self, testData):
        ...
        Macros.SelectReportInstance("Building Automation\\Decommissioned and Fault\\%s"%reportName)
    ...
    def _setupReportInstance(self):
        ...
        if Macros.isReportInstanceExisting("Building Automation\\Decommissioned and Fault\\%s"%reportName):
            Macros.SelectReportInstance("Building Automation\\Decommissioned and Fault\\%s"%reportName)
        else:
            Macros.SelectReportInstance("Building Automation\\Decommissioned and Fault")
        time.sleep(10)
