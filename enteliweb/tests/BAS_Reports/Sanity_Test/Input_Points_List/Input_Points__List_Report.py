...
# Global Settings
JSON_FILE_LOCATION = os.path.abspath(os.path.join(os.path.dirname(__file__), "Input_Points_List_Report.json"))
...
class TestCase(TestCaseTemplate):
    ...
    def testMain(self, testData):
        ...
        Macros.SelectReportInstance("Building Automation\\Input Points List\\%s"%reportName)
    ...
    def _setupReportInstance(self):
        ...
        if Macros.isReportInstanceExisting("Building Automation\\Input Points List\\%s"%reportName):
            Macros.SelectReportInstance("Building Automation\\Input Points List\\%s"%reportName)
        else:
            Macros.SelectReportInstance("Building Automation\\Input Points List")
        time.sleep(10)
