...
# Global Settings
JSON_FILE_LOCATION = os.path.abspath(os.path.join(os.path.dirname(__file__), "DER_Failure_Report.json"))
...
@ddt
class DERFailureReport(TestCaseTemplate):
    
    @classmethod
    def setUpClass(cls):
        super(DERFailureReport, cls).setUpClass()
    ...
    def tearDownClass(cls):
        super(DERFailureReport, cls).tearDownClass()
    
    def setUp(self):
        super(DERFailureReport, self).setUp()
    ...
    def tearDown(self):
        super(DERFailureReport, self).tearDown()
        ...
        for key, value in resultFromReport.iteritems():
            for item in value:
                ...
                for subKey, subValue in item.iteritems():
                    ...
                    if propertyName is None or propertyName in ("Object_Ref", "Exchange_Flags"):
                        continue
