...
# Global Settings
JSON_FILE_LOCATION = os.path.abspath(os.path.join(os.path.dirname(__file__), "Network_Number_Report.json"))
...
@ddt
class NetworkNumberReport(TestCaseTemplate):
    
    @classmethod
    def setUpClass(cls):
        super(NetworkNumberReport, cls).setUpClass()
    ...
    def tearDownClass(cls):
        super(NetworkNumberReport, cls).tearDownClass()
    
    def setUp(self):
        super(NetworkNumberReport, self).setUp()
    ...
    def tearDown(self):
        super(NetworkNumberReport, self).tearDown()
    ...
    def _test03(self, testData):
        ...
        for item in resultFromReport:
            ...
            for key, value in item.iteritems():
                ...
                expected = None
                if key in ("MS/TP NET1", "MS/TP NET2"):
                    groupObj = getattr(propertyValue, "mstp")
                    propertyObj = getattr(groupObj, "network")
                    expected = propertyObj.value
                elif key == "Local Net":
                    groupObj = getattr(propertyValue, "bacnet")
                    propertyObj = getattr(groupObj, "localNetworkNumber")
                    expected = propertyObj.value
                elif key == "Ethernet":
                    groupObj = getattr(propertyValue, "bnEther")
                    propertyObj = getattr(groupObj, "network")
                    expected = propertyObj.value
                else:
