...
# Global Settings
JSON_FILE_LOCATION = os.path.abspath(os.path.join(os.path.dirname(__file__), "block_commandmanual_request.json"))
...
class TestCase(TestCaseTemplate):
    ...
    @data(*getTestingData())
    def test01_command_wsbac(self, test_data):
        
        self.test_data = test_data
        self._testMethodDoc = "Request 'wsbac/commandmanual' for '%s' without ESignature will be blocked if feature is enabled with optional comment"%test_data.getObjRef()
    ...
    @data(*getTestingData())
    def test02_command_wsbac(self, test_data):
        
        self.test_data = test_data
        self._testMethodDoc = "Request 'wsbac/commandmanual' for '%s' without ESignature will be blocked if feature is enabled with mandatory comment"%test_data.getObjRef()
    ...
    @data(*getTestingData())    
    def test03_command_wsbac(self, test_data):
        
        self.test_data = test_data
        self._testMethodDoc = "Request 'wsbac/commandmanual' for '%s' with invalid ESignature will be blocked if feature is enabled with optional comment"%test_data.getObjRef()
        ...
        # send POST request
        self._wsbac_command_manual(test_data.site, test_data.device_number, test_data.object_type, test_data.instance_number, self.password)
    ...
    @data(*getTestingData())    
    def test04_command_wsbac(self, test_data):
        
        self.test_data = test_data
        self._testMethodDoc = "Request 'wsbac/commandmanual' for '%s' with invalid ESignature will be blocked if feature is enabled with mandatory comment"%test_data.getObjRef()
        ...
        # send POST request
        self._wsbac_command_manual(test_data.site, test_data.device_number, test_data.object_type, test_data.instance_number, self.password, "esign message")
    ...
    @data(*getTestingData())  
    def test05_command_wsbac(self, test_data):
        
        self.test_data = test_data
        self._testMethodDoc = "Request 'wsbac/commandmanual' for '%s' with valid ESignature will get through if feature is enabled with optional comment"%test_data.getObjRef()
        ...
        self._wsbac_command_manual(test_data.site, test_data.device_number, test_data.object_type, test_data.instance_number, password_base64)
    ...
    @data(*getTestingData())  
    def test06_command_wsbac(self, test_data):
        
        self.test_data = test_data
        self._testMethodDoc = "Request 'wsbac/commandmanual' for '%s' with valid ESignature and comment will get through if feature is enabled with mandatory comment"%test_data.getObjRef()
    ...
    @data(*getTestingData())    
    def test07_command_wsbac(self, test_data):
        
        self.test_data = test_data
        self._testMethodDoc = "Request 'wsbac/commandmanual' for '%s' with valid ESignature but no comment will be blocked if feature is enabled with mandatory comment"%test_data.getObjRef()
    ...
    @data(*getTestingData())    
    def test08_command_wsbac(self, test_data):
        
        self.test_data = test_data
        self._testMethodDoc = "Request 'wsbac/commandmanual' for '%s' without ESignature will get through if feature is disabled"%test_data.getObjRef()
    ...
    def _wsbac_command_manual(self, siteName, deviceNumber, objType, objInstance, esignature=None, comment=None):
        """ create an object on target device """
        obj_ref = "//%s/%s.%s%s"%(siteName, deviceNumber, objType, objInstance)
