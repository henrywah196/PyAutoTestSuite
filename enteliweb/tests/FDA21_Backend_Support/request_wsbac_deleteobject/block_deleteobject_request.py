# coding: utf-8
################################################################################################
# Test Case   : Implement Back end for ESignature and block deleteobject request (EWEB-22478)
#
# Description : 
#
################################################################################################
...
# Global Settings
JSON_FILE_LOCATION = os.path.abspath(os.path.join(os.path.dirname(__file__), "block_deleteobject_request.json"))
...
class TestCase(TestCaseTemplate):
    ...
    @data(*getTestingData())
    def test01_deleteobject_wsbac(self, test_data):
        
        self.test_data = test_data
        self._testMethodDoc = "Request 'wsbac/deleteobject' for '%s' without ESignature will be blocked if feature is enabled with optional comment"%test_data.getObjRef()
    ...
    def test02_deleteobject_wsbac(self, test_data):
        ...
        self.test_data = test_data
        self._testMethodDoc = "Request 'wsbac/deleteobject' for '%s' without ESignature will be blocked if feature is enabled with mandatory comment"%test_data.getObjRef()
    ...
    @data(*getTestingData())    
    def test03_deleteobject_wsbac(self, test_data):
        
        self.test_data = test_data
        self._testMethodDoc = "Request 'wsbac/deleteobject' for '%s' with invalid ESignature will be blocked if feature is enabled with optional comment"%test_data.getObjRef()
    ...
    def test04_deleteobject_wsbac(self, test_data):
        ...
        self.test_data = test_data
        self._testMethodDoc = "Request 'wsbac/deleteobject' for '%s' with invalid ESignature will be blocked if feature is enabled with mandatory comment"%test_data.getObjRef()
        ...
        # send POST request
        self._wsbac_deleteobject(test_data.site, test_data.device_number, test_data.object_type, test_data.instance_number, test_data.object_name, self.password, "esign message")
    ...
    @data(*getTestingData())    
    def test05_deleteobject_wsbac(self, test_data):
        
        self.test_data = test_data
        self._testMethodDoc = "Request 'wsbac/deleteobject' for '%s' with valid ESignature will get through if feature is enabled with optional comment"%test_data.getObjRef()
    ...
    @data(*getTestingData())    
    def test06_deleteobject_wsbac(self, test_data):
        
        self.test_data = test_data
        self._testMethodDoc = "Request 'wsbac/deleteobject' for '%s' with valid ESignature and comment will get through if feature is enabled with mandatory comment"%test_data.getObjRef()
        ...
        # send POST request
        password_base64 = base64.b64encode(self.password)
        self._wsbac_deleteobject(test_data.site, test_data.device_number, test_data.object_type, test_data.instance_number, test_data.object_name, password_base64, "esign message")
    ...
    @data(*getTestingData())    
    def test07_deleteobject_wsbac(self, test_data):
        
        self.test_data = test_data
        self._testMethodDoc = "Request 'wsbac/deleteobject' for '%s' with valid ESignature but no comment will be blocked if feature is enabled with mandatory comment"%test_data.getObjRef()
    ...
    @data(*getTestingData())    
    def test08_deleteobject_wsbac(self, test_data):
        
        self.test_data = test_data
        self._testMethodDoc = "Request 'wsbac/deleteobject' for '%s' without ESignature will get through if feature is disabled"%test_data.getObjRef()
    ...
    def _wsbac_deleteobject(self, siteName, deviceNumber, objType, objInstance, objName, esignature=None, comment=None):
        """ create an object on target device """
        url = "%s/wsbac/deleteobject"%self.base_url
        payload = {"ref"      : '["//%s/%s.%s%s"]'%(siteName, deviceNumber, objType, objInstance),
                   "_csrfToken"  : self.csrf_token}
        if esignature is not None:
            payload["esignature_password"] = esignature
