...
# Global Settings
JSON_FILE_LOCATION = os.path.abspath(os.path.join(os.path.dirname(__file__), "block_commandmanualvalue_request.json"))
...
class TestCase(TestCaseTemplate):
    ...
    def test0102_command_wsbac(self, test_data):
        ...
        self._test01(test_data, value_of_array=True)
    ...
    def test0202_command_wsbac(self, test_data):
        ...
        self._test02(test_data, value_of_array=True)
    ...
    def test0302_command_wsbac(self, test_data):
        ...
        self._test03(test_data, value_of_array=True)
    ...
    def test0402_command_wsbac(self, test_data):
        ...
        self._test04(test_data, value_of_array=True)
    ...
    def test0502_command_wsbac(self, test_data):
        ...
        self._test05(test_data, value_of_array=True)
    ...
    def test0602_command_wsbac(self, test_data):
        ...
        self._test06(test_data, value_of_array=True)
    ...
    def test0702_command_wsbac(self, test_data):
        ...
        self._test07(test_data, value_of_array=True)
    ...
    def test0802_command_wsbac(self, test_data):
        ...
        self._test08(test_data, value_of_array=True)
        
        
    def _test01(self, test_data, value_of_array=False):
        
        self.test_data = test_data
        if value_of_array:
            self._testMethodDoc = "Request 'wsbac/commandmanualvalue from object list' for '%s' without ESignature will be blocked if feature is enabled with optional comment"%test_data.getObjRef()
        else:
            self._testMethodDoc = "Request 'wsbac/commandmanualvalue from watchlist widget' for '%s' without ESignature will be blocked if feature is enabled with optional comment"%test_data.getObjRef()
    ...
    def _test02(self, test_data, value_of_array=False):
        
        self.test_data = test_data
        if value_of_array:
            self._testMethodDoc = "Request 'wsbac/commandmanualvalue from object list' for '%s' without ESignature will be blocked if feature is enabled with mandatory comment"%test_data.getObjRef()
        else:
            self._testMethodDoc = "Request 'wsbac/commandmanualvalue from watchlist widget' for '%s' without ESignature will be blocked if feature is enabled with mandatory comment"%test_data.getObjRef()
    ...
    def _test03(self, test_data, value_of_array=False):
        
        self.test_data = test_data
        if value_of_array:
            self._testMethodDoc = "Request 'wsbac/commandmanualvalue from object list' for %s with invalid ESignature will be blocked if feature is enabled with optional comment"%test_data.getObjRef()
        else:
            self._testMethodDoc = "Request 'wsbac/commandmanualvalue from watchlist widget' for %s with invalid ESignature will be blocked if feature is enabled with optional comment"%test_data.getObjRef()
        ...
        # send POST request
        if value_of_array:
            self._wsbac_command_manualvalue_array(test_data.site, test_data.device_number, test_data.object_type, test_data.instance_number, test_data.property_value, self.password)
        else:
            self._wsbac_command_manualvalue(test_data.site, test_data.device_number, test_data.object_type, test_data.instance_number, test_data.property_value, self.password)
    ...
    def _test04(self, test_data, value_of_array=False):
        
        self.test_data = test_data
        if value_of_array:
            self._testMethodDoc = "Request 'wsbac/commandmanualvalue from object list' for %s with invalid ESignature will be blocked if feature is enabled with mandatory comment"%test_data.getObjRef()
        else:
            self._testMethodDoc = "Request 'wsbac/commandmanualvalue from watchlist widget' for %s with invalid ESignature will be blocked if feature is enabled with mandatory comment"%test_data.getObjRef()
        ...
        # send POST request
        if value_of_array:
            self._wsbac_command_manualvalue_array(test_data.site, test_data.device_number, test_data.object_type, test_data.instance_number, test_data.property_value, self.password, "esign message")
        else:
            self._wsbac_command_manualvalue(test_data.site, test_data.device_number, test_data.object_type, test_data.instance_number, test_data.property_value, self.password, "esign message")
    ...
    def _test05(self, test_data, value_of_array=False):
        
        self.test_data = test_data
        if value_of_array:
            self._testMethodDoc = "Request 'wsbac/commandmanualvalue from object list' for %s with valid ESignature will get through if feature is enabled with optional comment"%test_data.getObjRef()
        else:
            self._testMethodDoc = "Request 'wsbac/commandmanualvalue from watchlist widget' for %s with valid ESignature will get through if feature is enabled with optional comment"%test_data.getObjRef()
        ...
        if value_of_array:
            self._wsbac_command_manualvalue_array(test_data.site, test_data.device_number, test_data.object_type, test_data.instance_number, test_data.property_value, password_base64)
        else:
            self._wsbac_command_manualvalue(test_data.site, test_data.device_number, test_data.object_type, test_data.instance_number, test_data.property_value, password_base64)
    ...
    def _test06(self, test_data, value_of_array=False):
        
        self.test_data = test_data
        if value_of_array:
            self._testMethodDoc = "Request 'wsbac/commandmanualvalue from object list' for %s with valid ESignature and comment will get through if feature is enabled with mandatory comment"%test_data.getObjRef()
        else:
            self._testMethodDoc = "Request 'wsbac/commandmanualvalue from watchlist widget' for %s with valid ESignature and comment will get through if feature is enabled with mandatory comment"%test_data.getObjRef()
    ...
    def _test07(self, test_data, value_of_array=False):
        
        self.test_data = test_data
        if value_of_array:
            self._testMethodDoc = "Request 'wsbac/commandmanualvalue from object list' for %s with valid ESignature but no comment will be blocked if feature is enabled with mandatory comment"%test_data.getObjRef()
        else:
            self._testMethodDoc = "Request 'wsbac/commandmanualvalue from watchlist widget' for %s with valid ESignature but no comment will be blocked if feature is enabled with mandatory comment"%test_data.getObjRef()
