...
# Global Settings
JSON_FILE_LOCATION = os.path.abspath(os.path.join(os.path.dirname(__file__), "block_commandauto_request.json"))
...
class TestCase(TestCaseTemplate):
    ...
    @data(*getTestingData())
    def test0101_command_wsbac(self, test_data):
        
        self._testMethodDoc = "Request 'wsbac/commandauto from object list' for '%s' without ESignature will be blocked if feature is enabled with optional comment"%test_data.getObjRef()
        self._test01(test_data, value_is_array=True)
        
        
    @data(*getTestingData())
    def test0102_command_wsbac(self, test_data):
        
        self._testMethodDoc = "Request 'wsbac/commandauto from watchlist widget' for '%s' without ESignature will be blocked if feature is enabled with optional comment"%test_data.getObjRef()
        self._test01(test_data)
        
        
    @data(*getTestingData())
    def test0201_command_wsbac(self, test_data):
        
        self._testMethodDoc = "Request 'wsbac/commandauto from object list' for '%s' without ESignature will be blocked if feature is enabled with mandatory comment"%test_data.getObjRef()
        self._test02(test_data, value_is_array=True)
        
        
    @data(*getTestingData())
    def test0202_command_wsbac(self, test_data):
        
        self._testMethodDoc = "Request 'wsbac/commandauto from watchlist widget' for '%s' without ESignature will be blocked if feature is enabled with mandatory comment"%test_data.getObjRef()
        self._test02(test_data)
        
        
    @data(*getTestingData())    
    def test0301_command_wsbac(self, test_data):
        
        self._testMethodDoc = "Request 'wsbac/commandauto from object list' for '%s' with invalid ESignature will be blocked if feature is enabled with optional comment"%test_data.getObjRef()
        self._test03(test_data, value_is_array=True)
        
        
    @data(*getTestingData())    
    def test0302_command_wsbac(self, test_data):
        
        self._testMethodDoc = "Request 'wsbac/commandauto from watchlist widget' for '%s' with invalid ESignature will be blocked if feature is enabled with optional comment"%test_data.getObjRef()
        self._test03(test_data)
        
        
    @data(*getTestingData())    
    def test0401_command_wsbac(self, test_data):
        
        self._testMethodDoc = "Request 'wsbac/commandauto from object list' for '%s' with invalid ESignature will be blocked if feature is enabled with mandatory comment"%test_data.getObjRef()
        self._test04(test_data, value_is_array=True)
        
        
    @data(*getTestingData())    
    def test0402_command_wsbac(self, test_data):
        
        self._testMethodDoc = "Request 'wsbac/commandauto from watchlist widget' for '%s' with invalid ESignature will be blocked if feature is enabled with mandatory comment"%test_data.getObjRef()
        self._test04(test_data)
        
    
    @data(*getTestingData())  
    def test0501_command_wsbac(self, test_data):
        
        self._testMethodDoc = "Request 'wsbac/commandauto from object list' for '%s' with valid ESignature will get through if feature is enabled with optional comment"%test_data.getObjRef()
        self._test05(test_data, value_is_array=True)
        
        
    @data(*getTestingData())  
    def test0502_command_wsbac(self, test_data):
        
        self._testMethodDoc = "Request 'wsbac/commandauto from watchlist widget' for '%s' with valid ESignature will get through if feature is enabled with optional comment"%test_data.getObjRef()
        self._test05(test_data)
        
        
    @data(*getTestingData())  
    def test0601_command_wsbac(self, test_data):
        
        self._testMethodDoc = "Request 'wsbac/commandauto from object list' for '%s' with valid ESignature and comment will get through if feature is enabled with mandatory comment"%test_data.getObjRef()
        self._test06(test_data, value_is_array=True)
        
        
    @data(*getTestingData())  
    def test0602_command_wsbac(self, test_data):
        
        self._testMethodDoc = "Request 'wsbac/commandauto from watchlist widget' for '%s' with valid ESignature and comment will get through if feature is enabled with mandatory comment"%test_data.getObjRef()
        self._test06(test_data)
        
        
    @data(*getTestingData())  
    def test0701_command_wsbac(self, test_data):
        
        self._testMethodDoc = "Request 'wsbac/commandauto from object list' for '%s' with valid ESignature but no comment will be blocked if feature is enabled with mandatory comment"%test_data.getObjRef()
        self._test07(test_data, value_is_array=True)
        
        
    @data(*getTestingData())  
    def test0702_command_wsbac(self, test_data):
        
        self._testMethodDoc = "Request 'wsbac/commandauto from watchlist widget' for '%s' with valid ESignature but no comment will be blocked if feature is enabled with mandatory comment"%test_data.getObjRef()
        self._test07(test_data)
        
        
    @data(*getTestingData())    
    def test0801_command_wsbac(self, test_data):
        
        self._testMethodDoc = "Request 'wsbac/commandauto from object list' for '%s' without ESignature will get through if feature is disabled"%test_data.getObjRef()
        self._test08(test_data, value_is_array=True)
        
        
    @data(*getTestingData())    
    def test0802_command_wsbac(self, test_data):
        
        self._testMethodDoc = "Request 'wsbac/commandauto from watchlist widget' for '%s' without ESignature will get through if feature is disabled"%test_data.getObjRef()
        self._test08(test_data)
        
        
    def _test01(self, test_data, value_is_array=False):
        ''' comment as option'''
        ...
        # send POST request
        if value_is_array:
            self._wsbac_command_auto_array(test_data.site, test_data.device_number, test_data.object_type, test_data.instance_number)
        else:
    ...
    def _test02(self, test_data, value_is_array=False):
    ...
    def _test03(self, test_data, value_is_array=False):
        ''' comment as option '''
        ...
        # send POST request
        if value_is_array:
            self._wsbac_command_auto_array(test_data.site, test_data.device_number, test_data.object_type, test_data.instance_number, self.password)
        else:
            self._wsbac_command_auto(test_data.site, test_data.device_number, test_data.object_type, test_data.instance_number, self.password)
    ...
    def _test04(self, test_data, value_is_array=False):
        ''' comment as mandatory '''
        ...
        # send POST request
        if value_is_array:
            self._wsbac_command_auto_array(test_data.site, test_data.device_number, test_data.object_type, test_data.instance_number, self.password, "esign message")
        else:
            self._wsbac_command_auto(test_data.site, test_data.device_number, test_data.object_type, test_data.instance_number, self.password, "esign message")
    ...
    def _test05(self, test_data, value_is_array=False):
        ''' comment as option '''
        ...
        if value_is_array:
            self._wsbac_command_auto_array(test_data.site, test_data.device_number, test_data.object_type, test_data.instance_number, password_base64)
        else:
            self._wsbac_command_auto(test_data.site, test_data.device_number, test_data.object_type, test_data.instance_number, password_base64)
    ...
    def _test06(self, test_data, value_is_array=False):
        ''' comment as mandatory '''
    ...
    def _test07(self, test_data, value_is_array=False):
        ''' comment as mandatory '''
    ...
    def _test08(self, test_data, value_is_array=False):
    ...
    def _wsbac_command_auto_array(self, siteName, deviceNumber, objType, objInstance, esignature=None, comment=None):
        """ create an object on target device """
        obj_ref = "//%s/%s.%s%s"%(siteName, deviceNumber, objType, objInstance)
    ...
    def _wsbac_command_auto(self, siteName, deviceNumber, objType, objInstance, esignature=None, comment=None):
        """ create an object on target device """
        obj_ref = "//%s/%s.%s%s"%(siteName, deviceNumber, objType, objInstance)
        url = "%s/wsbac/commandauto"%self.base_url
        payload = {"format"      : "JSON",
