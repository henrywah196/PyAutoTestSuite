...
# Global Settings
JSON_FILE_LOCATION = os.path.abspath(os.path.join(os.path.dirname(__file__), "block_wsbacv3_command_request.json"))
...
def getTestingData():
    ...
    for item in json_data:
        ...
        myTestData.command = item["command"]
        myTestData.commandvalue = item["commandvalue"]
...
class TestCase(TestCaseTemplate):
    ...
    def test01_command_wsbac(self, test_data):
        ...
        self._testMethodDoc = "Request 'wsbacv3/command' for '%s' without ESignature will be blocked if feature is enabled with optional comment"%test_data.getObjRef()
        ...
        # send POST request
        self._wsbacv3_command(test_data.site, test_data.getObjRef(), test_data.command, test_data.commandvalue)
    ...
    def test02_command_wsbac(self, test_data):
        ...
        self.test_data = test_data
        self._testMethodDoc = "Request 'wsbacv3/command' for '%s' with invalid ESignature will be blocked if feature is enabled with optional comment"%test_data.getObjRef()
        ...
        # send POST request
        self._wsbacv3_command(test_data.site, test_data.getObjRef(), test_data.command, test_data.commandvalue, self.password)
    ...
    def test03_command_wsbac(self, test_data):
        ...
        self.test_data = test_data
        self._testMethodDoc = "Request 'wsbacv3/command' for '%s' with valid ESignature will get through if feature is enabled with optional comment"%test_data.getObjRef()
        ...
        self._wsbacv3_command(test_data.site, test_data.getObjRef(), test_data.command, test_data.commandvalue, password_base64)
    ...
    def test04_command_wsbac(self, test_data):
        ...
        self.test_data = test_data
        self._testMethodDoc = "Request 'wsbac/command' for '%s' without ESignature will get through if feature is disabled with optional comment"%test_data.getObjRef()
    ...
    def _wsbacv3_command(self, siteName, obj_ref, command, command_value, esignature=None, comment=None):
        """ create an object on target device """
        url = "%s/wsbacv3/command"%self.base_url
        payload = {"input"        : obj_ref,
                   "command"      : command,
                   "site"         : "//%s/"%siteName,
