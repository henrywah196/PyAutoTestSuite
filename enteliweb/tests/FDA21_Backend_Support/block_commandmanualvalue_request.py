...
class TestCase(TestCaseTemplate):
    ...
    def _test01(self, test_data, value_of_array=False):
        ...
        if value_of_array:
            self._testMethodDoc = "Request 'wsbac/commandmanualvalue from object list' for '%s' without ESignature will be blocked if feature is enabled"%test_data.getObjRef()
        else:
            self._testMethodDoc = "Request 'wsbac/commandmanualvalue from watchlist widget' for '%s' without ESignature will be blocked if feature is enabled"%test_data.getObjRef()
    ...
    def _test02(self, test_data, value_of_array=False):
        ...
        self.test_data = test_data
        if value_of_array:
            self._testMethodDoc = "Request 'wsbac/commandmanualvalue from object list' for %s with invalid ESignature will be blocked if feature is enabled"%test_data.getObjRef()
        else:
            self._testMethodDoc = "Request 'wsbac/commandmanualvalue from watchlist widget' for %s with invalid ESignature will be blocked if feature is enabled"%test_data.getObjRef()
    ...
    def _test03(self, test_data, value_of_array=False):
        ...
        self.test_data = test_data
        if value_of_array:
            self._testMethodDoc = "Request 'wsbac/commandmanualvalue from object list' for %s with valid ESignature will get through if feature is enabled"%test_data.getObjRef()
        else:
            self._testMethodDoc = "Request 'wsbac/commandmanualvalue from watchlist widget' for %s with valid ESignature will get through if feature is enabled"%test_data.getObjRef()
    ...
    def _wsbac_command_manualvalue_array(self, siteName, deviceNumber, objType, objInstance, objValue, esignature=None):
        """ create an object on target device """
    ...
    def _wsbac_command_manualvalue(self, siteName, deviceNumber, objType, objInstance, objValue, esignature=None):
        """ create an object on target device """
        url = "%s/wsbac/commandmanual"%self.base_url
        payload = {"format"        : "JSON",
                   "ObjRef"        : "//%s/%s.%s%s"%(siteName, deviceNumber, objType, objInstance),
