...
# Global Settings
JSON_FILE_LOCATION = os.path.abspath(os.path.join(os.path.dirname(__file__), "Door_Activity_Report.json"))
...
class TestCase(TestCaseTemplate):
    ...
    def testMain(self, testData):
        ...
        Macros.SelectReportInstance("Access Control\\Door Activity\\%s"%reportName)
        ...
        # Prepare test helper
        self.resultFromHelper = AccessActivity.get_filtered_events(testData, timestamp_format=True, group_by="Door")
        ...
        if not self.resultFromHelper:
            ...
            for key, value in self.resultFromReport.iteritems():
                if key in self.resultFromHelper:
                    ...
                    self.perform(self.assertItemsEqual, current, expected, "verify the accuracy of returned events for Door '%s' failed"%key)
    ...
    def _setupReportInstance(self):
        ...
        if Macros.isReportInstanceExisting("Access Control\\Door Activity\\%s"%reportName):
            Macros.SelectReportInstance("Access Control\\Door Activity\\%s"%reportName)
        else:
            Macros.SelectReportInstance("Access Control\\Door Activity")
        time.sleep(10)
