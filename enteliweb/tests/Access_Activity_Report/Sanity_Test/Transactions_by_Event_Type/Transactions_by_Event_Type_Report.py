...
# Global Settings
JSON_FILE_LOCATION = os.path.abspath(os.path.join(os.path.dirname(__file__), "Transactions_by_Event_Type_Report.json"))
...
class TestCase(TestCaseTemplate):
    ...
    def testMain(self, testData):
        ...
        Macros.SelectReportInstance("Access Control\\Transactions by Event Type\\%s"%reportName)
        ...
        # Prepare test helper
        self.resultFromHelper = AccessActivity.get_filtered_events(testData, timestamp_format=True, group_by="Event Type")
        ...
        if not self.resultFromHelper:
            ...
            for key, value in self.resultFromReport.iteritems():
                if key in self.resultFromHelper:
                    ...
                    self.perform(self.assertItemsEqual, current, expected, "verify the accuracy of returned events for Event Type '%s' failed"%key)
    ...
    def _setupReportInstance(self):
        ...
        if Macros.isReportInstanceExisting("Access Control\\Transactions by Event Type\\%s"%reportName):
            Macros.SelectReportInstance("Access Control\\Transactions by Event Type\\%s"%reportName)
        else:
            Macros.SelectReportInstance("Access Control\\Transactions by Event Type")
        time.sleep(10)
