...
# Global Settings
JSON_FILE_LOCATION = os.path.abspath(os.path.join(os.path.dirname(__file__), "Card_Access_Activity_Report.json"))
...
class TestCase(TestCaseTemplate):
    ...
    def testMain(self, testData):
        ...
        self.testingReport.cardsDelete.click()
        self.testingReport.cardsEdit.click()
        time.sleep(3)
        result = self.testingReport.findCardUsersWindow.isDisplayed()
        self.assertTrue(result, "find card users window doesn't popup")
        
        result = self.testingReport.findCardUsersWindow.getTotalPageNumber()
        print "total pages: %s"%result
        
        result = self.testingReport.findCardUsersWindow.getCurrentPageNumber()
        print "current page: %s"%result
        
        self.testingReport.findCardUsersWindow.setCurrentPage("10")
        
        result = self.testingReport.findCardUsersWindow.getCurrentPageNumber()
        print "current page: %s"%result
        
        self.testingReport.findCardUsersWindow.btnByName.click()
        time.sleep(3)
        
        self.testingReport.findCardUsersWindow.btnByKeyword.click()
        
        self.testingReport.findCardUsersWindow.filterBy = "Abbo"
        
        self.testingReport.findCardUsersWindow.btnByName.click()
        time.sleep(3)
        
        result = self.testingReport.findCardUsersWindow.getObjectsList()
        print result
        
        self.testingReport.findCardUsersWindow.selectObject(3)
        
        self.testingReport.findCardUsersWindow.selectObject(["Abbott, Roberta", "CU17386"])
        
        self.testingReport.findCardUsersWindow.btnOK.click()
        '''
        ...
        # Prepare test helper
        self.resultFromHelper = AccessActivity.get_filtered_events(testData, timestamp_format=True)
        ...
        if not self.resultFromHelper:
            ...
            self.assertItemsEqual(self.resultFromReport, self.resultFromHelper, "verify the accuracy of returned events failed")
