...
class HospitalityPageObj(HotSpotBuildingObj):
    
    def __repr__(self):
        super(HospitalityPageObj, self).__repr__()
    ...
    def isLoaded(self):
        ...
        result = True
        try: 
            assert super(HospitalityPageObj, self).isLoaded() is True
            
            elem = self.image.getElement(self.image.locator)
            img = elem.find_element_by_tag_name("img")
            src = img.get_attribute("src")
            assert ("hotel-stillhiglhight2.jpg" in src) is True
